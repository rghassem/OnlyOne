...
function resize(resizeRenderer: boolean = false) {
  ...
  const actualHeight =
    letterStageHeight + ScoringAreaOffset + scoringAreaHeight;
...
function initLevel() {
  ...
  const forceLevel =
    paramLevel && parseInt(paramLevel) ? parseInt(paramLevel) : null;

  currentLevel =
    forceLevel || Number(window.localStorage.getItem('level')) || 0;
...
async function start() {
  ...
  if (EnableSolver) {
    solveButton = makeButton(app.stage, 80, 28, 'Solve', async () => {
      ...
      while (
        solution.solved &&
        solution.bestPath.moves.length > 0 &&
        !checkWin(gameboard)
      ) {
